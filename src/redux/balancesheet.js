...
export const balanceSheetThunk = createAsyncThunk(
    "data/balanceSheet",
    async (ticker, thunkAPI) => {
        ...
        try {
            const res = await axios.get(`${url}finance`, {
...
const balanceSheetSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(balanceSheetThunk.pending, (state) => {
            state.loading = true
        })
        builder.addCase(balanceSheetThunk.fulfilled, (state, action) => {
            state.loading = false;
            state.data = action.payload
            state.error = null
        })
        builder.addCase(balanceSheetThunk.rejected, (state, action) => {
            state.loading = false;
